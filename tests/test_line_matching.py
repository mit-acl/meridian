...
def test_line_distance_1(default_matcher_params, three_line_segments):
    ...
    M_0a0b_1a2b = M[0, 5] 
    M_0a0b_1a2b_expected_dist_diff = np.exp(-((1**2))/(2*matcher.params.sigma_dist**2))
    M_0a0b_1a2b_expected_angle_diff = np.exp(-((np.deg2rad(45)**2))/(2*matcher.params.sigma_angle_rad**2))
    M_0a0b_1a2b_expected = np.sqrt(M_0a0b_1a2b_expected_dist_diff * M_0a0b_1a2b_expected_angle_diff)
...
def test_line_distance_with_semantics_1(default_matcher_params, three_line_segments):
    ...
    M_0a0b_1a2b_expected_dist_diff = np.exp(-((1**2))/(2*matcher.params.sigma_dist**2))
    M_0a0b_1a2b_expected_angle_diff = np.exp(-((np.deg2rad(45)**2))/(2*matcher.params.sigma_angle_rad**2))
    M_0a0b_1a2b_expected_pairwise = np.sqrt(M_0a0b_1a2b_expected_dist_diff * M_0a0b_1a2b_expected_angle_diff)
    M_0a0b_1a2b_expected_cosine1 = np.dot(linesa[0].cos_feature, linesb[0].cos_feature) / (np.linalg.norm(linesa[0].cos_feature) * np.linalg.norm(linesb[0].cos_feature))
    M_0a0b_1a2b_expected_cosine2 = np.dot(linesa[1].cos_feature, linesb[2].cos_feature) / (np.linalg.norm(linesa[1].cos_feature) * np.linalg.norm(linesb[2].cos_feature))
    M_0a0b_1a2b_expected = (M_0a0b_1a2b_expected_pairwise * M_0a0b_1a2b_expected_cosine1 * M_0a0b_1a2b_expected_cosine2)**(1/3)
    print(M_0a0b_1a2b, M_0a0b_1a2b_expected)
    assert pytest.approx(M_0a0b_1a2b) == M_0a0b_1a2b_expected
