...
Adapted from MMDA's viz_lewis_result.py to work with gen_seg_match pipeline
output (PoseEstimationResultMatrix .npz files + submap .pkl files).

Usage:
    python -m gen_seg_match.viz.multi_hypothesis_viz \
...
def plot_segments_px(ax, segments, px_per_m, crop_origin_px,
                     color='k', alpha=1.0, lw=1.5, ms=4, label=None):
    ...
    for seg in segments:
        ...
        if isinstance(seg, SegmentLine):
            ...
            if ep0 is not None and ep1 is not None:
                ...
                ax.plot([p0_px[0], p1_px[0]], [p0_px[1], p1_px[1]], '-',
...
def transform_segments_2d(segments, T):
    ...
    for seg in segments:
        ...
        if isinstance(seg, SegmentLine):
            ...
            out.append(SegmentLine(
                id=seg.id, point=pt_new, direction=dir_new,
                endpoints=(ep0_new, ep1_new),
                ratio_feature=seg.ratio_feature, cos_feature=seg.cos_feature,
            ))
        elif isinstance(seg, SegmentPoint):
            out.append(SegmentPoint(
                id=seg.id, point=pt_new,
                ratio_feature=seg.ratio_feature, cos_feature=seg.cos_feature,
            ))
    return SegmentList(out)


def draw_associations_px(ax, associations, aerial_segs, ground_segs, T,
                         px_per_m, crop_origin_px):
    ...
    for k, (g_id, a_id) in enumerate(associations):
        ...
        color = pair_colors[k]
        ax.plot([g_px[0], a_px[0]], [g_px[1], a_px[1]], '--', color=color,
                lw=1.5, alpha=0.8)
        ax.plot(g_px[0], g_px[1], 's', color=color, ms=8,
                markeredgecolor='k', markeredgewidth=0.5, zorder=5)
        ax.plot(a_px[0], a_px[1], 'o', color=color, ms=8,
                markeredgecolor='k', markeredgewidth=0.5, zorder=5)


def draw_pose_px(ax, T, px_per_m, crop_origin_px, color='red',
                 arrow_len_m=5.0, lw=3):
    ...
    ax.annotate('', xy=pos_px + arrow_len_px * x_dir, xytext=pos_px,
                arrowprops=dict(arrowstyle='->', color=color, lw=lw),
                annotation_clip=False)
    ax.annotate('', xy=pos_px + arrow_len_px * y_dir, xytext=pos_px,
                arrowprops=dict(arrowstyle='->', color=color, lw=lw,
                                linestyle='--'),
                annotation_clip=False)
...
def visualize_multi_hypothesis(
    ...
    print(f"Aerial image: {data.aerial_img.shape}, px_per_m={px_per_m:.1f}, "
    ...
    if output_dir is None:
        output_dir = results_dir / "viz_multi_hyp" / f"ground_{ground_idx}_aerial_{aerial_key}"
    ...
    plot_segments_px(ax, aerial_segs, display_px_per_m, display_crop_origin,
                     color='black', alpha=0.4, lw=1, ms=3)
    ...
    for rank_i in range(top_k):
        ...
        draw_pose_px(ax, T, display_px_per_m, display_crop_origin,
                     color=color, arrow_len_m=5.0, lw=2)

    if has_gt:
        draw_pose_px(ax, T_gt, display_px_per_m, display_crop_origin,
                     color='lime', arrow_len_m=5.0, lw=3)
    ...
    for rank_i in range(top_k):
        ...
        ax.imshow(crop_rgb, alpha=0.8)
        plot_segments_px(ax, aerial_segs, display_px_per_m, display_crop_origin,
                         color='black', alpha=0.3, lw=1, ms=2)
        ...
        if not np.any(np.isnan(T)):
            ...
            plot_segments_px(ax, transformed, display_px_per_m, display_crop_origin,
                             color=colors[rank_i], alpha=0.8, lw=2, ms=5)
            draw_pose_px(ax, T, display_px_per_m, display_crop_origin,
                         color='red', arrow_len_m=5.0, lw=3)
            ...
            ax.set_title(f'Rank {rank_i+1}: {n_assoc} assoc, '
                         f't=({T[0,3]:.1f},{T[1,3]:.1f}), '
                         f'yaw={yaw:.1f}deg{gt_str}', fontsize=9)
        else:
    ...
    plt.suptitle(f'Individual Panels: ground {ground_idx}, aerial ({i_a},{j_a})',
                 fontsize=14)
    plt.tight_layout()
    ...
    for rank_i in range(top_k):
        ...
        ax.imshow(crop_rgb, alpha=0.8)
        plot_segments_px(ax, aerial_segs, display_px_per_m, display_crop_origin,
                         color='black', alpha=0.5, lw=1.0, ms=3, label='Aerial')
        transformed = transform_segments_2d(ground_segs, T)
        plot_segments_px(ax, transformed, display_px_per_m, display_crop_origin,
                         color='steelblue', alpha=0.7, lw=1.5, ms=4,
                         label='Ground (transformed)')
        draw_associations_px(ax, assoc, aerial_segs, ground_segs, T,
                             display_px_per_m, display_crop_origin)
        draw_pose_px(ax, T, display_px_per_m, display_crop_origin,
                     color='red', arrow_len_m=5.0, lw=3)
        ax.set_xlim(0, W)
        ax.set_ylim(H, 0)
        ax.legend(loc='upper right', fontsize=8)
        ax.set_title(f'Rank {rank_i+1}: {n_assoc} associations, '
    ...
    if has_gt:
        ...
        print(f"Best by SE(2) error: rank {best_idx+1}/{n_hyp}, "
...
def main():
    ...
    parser.add_argument("-g", "--ground-idx", type=int, required=True,
                        help="Ground submap index")
    parser.add_argument("-a", "--aerial-key", type=str, default=None,
                        help="Aerial patch key 'i_j' (default: auto-select most hypotheses)")
    parser.add_argument("-k", "--top-k", type=int, default=10,
                        help="Number of top hypotheses to show")
    ...
    parser.add_argument("--overlay-only", action="store_true",
                        help="Only generate the overlay plot")
