...
def draw_line_on_img(img, line: SegmentLine, camera_params: rdpc.CameraParams,
                     color=(0,255,0), thickness=2):
    if line.num_endpoints == 0:
        ...
        points_in_3d = [line.endpoints[0] + (line.endpoints[1] - line.endpoints[0]) * i for i in np.linspace(0, 1, 100)]
    points_in_2d = [rdpc.xyz_2_pixel(p.reshape((3,1)), camera_params.K) for p in points_in_3d if p[2] > 0]
    for p in points_in_3d:
        if p[2] < 0: continue
    for i in range(len(points_in_2d)-1):
...
def draw_segment_types_on_img(img, segments, camera_params: rdpc.CameraParams, write_ids=False):
    ...
    for i, seg in enumerate(segments):
        if type(seg) == SegmentPoint:
            ...
            if write_ids:
                cv.putText(img, str(seg.id), (int(px[0])+10, int(px[1])+10), cv.FONT_HERSHEY_SIMPLEX, 1, (255,255,255), 2)
