...
def get_segments_with_occlusion(raw_observations: List[Observation], depth_img: np.ndarray, 
                                camera_params: rdpc.CameraParams) -> List[Segment]:
    ...
    for observation in raw_observations:
        occluded_points = observation.point_cloud[observation.point_cloud[:, 2] > depth_mask_thresh, :]
        ...
        occluded_pixels_3d = rdpc.pixel_depth_2_xyz(occluded_pixels[:,1], occluded_pixels[:,0], occluded_pixels_depths, camera_params.K).T

        # print(occluded_points.shape)
        # print(occluded_pixels_3d.shape)

        occluded_points = np.vstack([occluded_points, occluded_pixels_3d]) if occluded_points.shape[0] > 0 else occluded_pixels_3d
        # print(f"final {occluded_points.shape=}")
    ...
    return segments
    # print([seg.linearity for seg in segments])
    # print([seg.planarity for seg in segments])
    # print([seg.scattering for seg in segments])
    # for seg in segments:
    #     if is_line(seg):
    #         seg.id = 1
    #     elif is_plane(seg):
    ...
    for seg in segments:
        if is_line(seg):
            ...
            if end_pts[0] is not None:
                general_segments.append(SegmentPoint(id=0, point=end_pts[0].flatten()))
            if end_pts[1] is not None:
                general_segments.append(SegmentPoint(id=0, point=end_pts[1].flatten()))
            general_segments.append(SegmentLine(id=seg.id, point=offset.flatten(), direction=vector.flatten()))
        elif is_plane(seg):
            pass
        else:
            general_segments.append(SegmentPoint(id=seg.id, point=seg.center.flatten()))
