...
class LineCloud:
    ...
    def centers_and_directions(self):
        ...
        directions = np.array([(line[1] - line[0]) / np.linalg.norm(line[1] - line[0]) for line in self.lines])
    ...
    def plot(self, color='random', ax=None):
        ...
        for line in self.lines:
            p1, p2 = line
            line_color = color if color != 'random' else np.random.rand(3,)
            ...
            while len(untouched_lines) > 0:
                ...
                for i, candidate_line in enumerate(untouched_lines):
                    ...
                    if angle < self.params.min_merge_angle_deg:
                        ...
                        min_dist = min(dists)
                        if np.linalg.norm(center_p - center_q) < self.params.min_merge_dist:
                            untouched_lines.pop(i)
                            mean_dir_unnormalized_option_1 = (dir_p + dir_q)
                            mean_dir_unnormalized_option_2 = (dir_p - dir_q)
                            if np.linalg.norm(mean_dir_unnormalized_option_1) > np.linalg.norm(mean_dir_unnormalized_option_2):
