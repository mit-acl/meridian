...
class AssociationVizParams:
    ...
    def aerial_img_aspect_ratio(self) -> float:
        if self.aerial_img_crop == (0, 0, -1, -1):
            self.aerial_img_crop = (0, 0, self.aerial_img.shape[1], self.aerial_img.shape[0])
    ...
    def aerial_viz_height(self) -> int:
        if self.frame_orientation == FrameOrientation.SIDE_BY_SIDE:
            ...
            width = self.ground_img_data.width
            height = self.ground_img_data.height + int(width / self.aerial_img_aspect_ratio)
        else:
            raise ValueError(f"Unknown frame orientation: {self.frame_orientation}")
        return height
...
class AssociationViz:
    ...
    def create_video(self):
        ...
        out = cv.VideoWriter(self.params.output_path, fourcc, self.params.fps, 
                             (self.params.output_img_shape[1], self.params.output_img_shape[0]))

        aerial_img = self.params.cropped_img.copy()
        aerial_img = cv.resize(aerial_img, (self.params.aerial_viz_width, self.params.aerial_viz_height))

        aerial_outlines = []
        for seg in self.params.matched_aerial_segments:
            convex_hull = seg.convex_hull_pixels(img_pixel_scale=self.params.cropped_img_pixel_scale, 
                                                 img_origin_m=self.params.aerial_crop_origin_m)
            aerial_outlines.append(convex_hull)
            cv.polylines(aerial_img, [convex_hull], True, seg.viz_color[::-1], self.params.line_width)
        ...
        for t in np.arange(t0, tf, 1 / self.params.fps):
            ...
            for i in range(len(self.params.matched_ground_segments)):
                ...
                if np.linalg.norm(seg.center.flatten() - 
                    ...
                    if seg_outline is None:
                        continue
                    ground_img = visualize_segment_on_img(seg, ground_pose_t, ground_img, show_id=self.params.show_segment_ids)
                    seg_seen[i] = True
                    ground_outlines[i] = seg_outline + self.params.ground_vid_pixel_origin
                    ground_outlines[i] = ground_outlines[i].astype(np.int32)
                        
            
            combined_img[self.params.ground_vid_pixel_origin[1]:, 
                              self.params.ground_vid_pixel_origin[0]:] = ground_img
            ...
            for i, seg in enumerate(self.params.matched_ground_segments):
                ...
                cv.line(combined_img, tuple(nearest_pixels[0].astype(np.int32)), 
                        tuple(nearest_pixels[1].astype(np.int32)), seg.viz_color[::-1], 
