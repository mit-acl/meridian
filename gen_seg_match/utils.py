...
def sort_time_intervals(intervals: Union[List[Tuple[float, float]], np.ndarray], ref_time: float) -> np.ndarray:
    """
    ...
    contains_ref_avg_times = (intervals[contains_ref][:, 0] + intervals[contains_ref][:, 1]) / 2
    ...
    not_contains_ref_indices = not_contains_ref_indices[np.argsort(not_contains_ref_distances)]
