...
class PoseEstimationResultMatrix(np.ndarray):
    ...
    def plot(self, dpi: int = 500):
        ...
        mp = ax[0, 1].imshow(np.rad2deg(self.gt_rotation_diff_rad), cmap='magma', vmin=0)
        fig.colorbar(mp, fraction=0.04, pad=0.04)
        ...
        if show_sim:
            mp = ax[2, 1].imshow(self.descriptor_similarity, cmap='viridis', vmin=0.0, vmax=1.0)
            fig.colorbar(mp, fraction=0.04, pad=0.04)
    ...
    def plot_point_vs_line_associations(self):
        ...
        mp = ax[0].imshow(
            self.num_point_associations, cmap="viridis", vmin=0
        )
        fig.colorbar(mp, fraction=0.04, pad=0.04)
        mp = ax[1].imshow(
            self.num_line_associations, cmap="viridis", vmin=0
        )
        fig.colorbar(mp, fraction=0.04, pad=0.04)
