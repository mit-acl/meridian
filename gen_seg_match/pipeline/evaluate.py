...
class SubmapAlignEvaluator():
    ...
    def load_results(self, eval_inputs: Union[EvalInput, List[EvalInput]]):
        ...
        for eval_input in eval_inputs:
            ...
            for path, robots in zip(result_paths, self.params.robot_pairs):
                combined_results.append(PoseEstimationResultMatrix.load(path).reshape(-1))

            self.results[eval_input.get_name()] = PoseEstimationResultMatrix.concatenate(combined_results)
    ...
    def evaluate_align_success_rate(
        ...
        for name, results in self.results.items():
            ...
            correct_alignments = (
                ...
                (results.angle_error_rad <= np.deg2rad(self.params.angular_err_thresh_deg))
            )
    ...
    def _get_named_results(self, names: List[str] = None) -> Dict[str, PoseEstimationResultMatrix]:
        if names is None:
            return self.results
        else:
