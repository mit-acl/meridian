...
def submaps_from_roman_map(roman_map: ROMANMap, submap_params: SubmapParams) -> List[Submap]:
    """
    ...
    if submap_params.creation_method == 'force_fill':
        ...
        segments_sorted_by_time = sorted(roman_map.segments, key=lambda seg: seg.reference_time(use_avg_time=submap_params.segment_avg_time))
    ...
    # create submaps adaptively, then use metrics to fill -----------
        ...
        for i, (pose, t) in enumerate(zip(roman_map.trajectory, roman_map.times)):
            if i == 0 or np.linalg.norm(pose[:-1, -1] - submaps[-1].pose_flu[:-1, -1]) > submap_params.center_dist \
    ...
    # create submaps at set times -----------
        ...
        segment_time_intervals = np.array([(seg.first_seen, seg.last_seen) for seg in roman_map.segments])
        ...
        raise ValueError(f"Unknown submap creation method: {submap_params.creation_method}")
    ...
    for submap in submaps:
        ...
        submap.segments = [GeneralSegment.from_roman_segment(seg) for seg in submap.segments]
