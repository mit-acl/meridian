...
from meridian.map3d.voxel_grid import VoxelGrid
...
class MapSegment:
    def __init__(
        ...
        id: int = 0,
        params: MapSegmentParams = MapSegmentParams(),
    ):
        self.id = id
        self.dim = 3
    ...
    def update_from_segment(self, segment):
        ...
        if segment.semantic_descriptor is not None:
            self._add_semantic_descriptor(
                segment.semantic_descriptor, segment.semantic_descriptor_cnt
            )
    ...
    def _integrate_points_from_observation(self, observation: Observation):
        ...
        points_obs_world = Rwb @ points_obs_body + np.repeat(
            twb, num_points_obs, axis=1
        )
        ...
        if (
            ...
            occ_body = observation.occluded_points.T
            num_occ = occ_body.shape[1]
    ...
    def volume(self):
        if (
            self.num_points > 4
        ):  # 4 is the minimum number of points needed to define a 3D box
    ...
    def reconstruct_mask(self, pose, downsample_factor=1):
        if self._mask is None or not np.allclose(pose, self._mask_last_pose):
            mask = np.zeros(
                (self.camera_params.height, self.camera_params.width), dtype=np.uint8
            )
    ...
    def _pixels_2d(self, pose):
        if self._pixels is None or not np.allclose(pose, self._pixels_past_pose):
            ...
            pixels = pixels[
                np.bitwise_and(
                    pixels[:, 0] >= 0, pixels[:, 0] < self.camera_params.width
                ),
                :,
            ]
            pixels = pixels[
                np.bitwise_and(
                    pixels[:, 1] >= 0, pixels[:, 1] < self.camera_params.height
                ),
                :,
            ]
            self._pixels = pixels
    ...
    def reprojected_bbox(self, pose):
        ...
        lower_right = np.min(
            [
                np.max(pixels, axis=0).astype(int),
    ...
    def propagated_last_mask(self, t, pose, downsample_factor=1):
        ...
        points_all_cm1 = transform(
            np.linalg.inv(self.last_observation.pose), self.points, axis=0
        )
        ...
        points_cm1 = np.array(
            [
                pixel_depth_2_xyz(p[0], p[1], depth, self.camera_params.K)
                for p in points_uvm1
            ]
        )
        ...
        mask = cv.warpAffine(
            mask_unchanged.astype(np.float32),
            T_pixels[:2, :3],
            (mask_unchanged.shape[1], mask_unchanged.shape[0]),
        )
    ...
    def outline_2d(self, pose):
        if self._convex_hull is None or not np.allclose(
            pose, self._convex_hull_last_pose
        ):
            pixels = self._pixels_2d(pose)
            if pixels is None:
                return None
