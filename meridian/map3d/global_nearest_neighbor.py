...
def global_nearest_neighbor(
    ...
    similarity_range: np.ndarray = None,
):
    """
        ...
        for j in range(len2):
            ...
            if np.any(similarity < similarity_range[0, :]):
                ...
                if isinstance(similarity, np.ndarray):
                    sim_norm = (similarity - similarity_range[0, :]) / (
                        similarity_range[1, :] - similarity_range[0, :]
                    )
                    score = -np.power(
                        np.prod(sim_norm), 1.0 / len(similarity)
                    )  # geometric mean
                else:
    ...
    hungarian_cost = np.concatenate(
        [
            ...
            np.ones((scores.shape[0], 2 * scores.shape[1])),
        ],
        axis=0,
    )
