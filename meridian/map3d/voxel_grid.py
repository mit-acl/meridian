...
class VoxelGrid:
    ...
    def intersection(self, other):
        ...
        self_sub_grid = self.voxels[
            min_corner_int.item(0) - self.min_corner_int.item(0) : max_corner_int.item(
                0
            )
            - self.min_corner_int.item(0),
            min_corner_int.item(1) - self.min_corner_int.item(1) : max_corner_int.item(
                1
            )
            - self.min_corner_int.item(1),
            min_corner_int.item(2) - self.min_corner_int.item(2) : max_corner_int.item(
                2
            )
            - self.min_corner_int.item(2),
        ]

        other_sub_grid = other.voxels[
            min_corner_int.item(0) - other.min_corner_int.item(0) : max_corner_int.item(
                0
            )
            - other.min_corner_int.item(0),
            min_corner_int.item(1) - other.min_corner_int.item(1) : max_corner_int.item(
                1
            )
            - other.min_corner_int.item(1),
            min_corner_int.item(2) - other.min_corner_int.item(2) : max_corner_int.item(
                2
            )
            - other.min_corner_int.item(2),
        ]

        assert self_sub_grid.shape == other_sub_grid.shape, (
            "Subgrids do not have the same shape"
        )
    ...
    def iou(self, other, iom_as_iou=False):
        ...
        if iom_as_iou:
            if np.minimum(self.volume, other.volume) == 0:
                return 0.0
            return intersection / np.minimum(self.volume, other.volume)
        else:
            if (self.volume + other.volume - intersection) == 0:
                return 0.0
    ...
    def from_points(cls, points: np.ndarray, voxel_size: float):
        ...
        """
        min_corner = np.array(
            [np.floor(np.min(points, axis=0) / voxel_size) * voxel_size]
        )
        max_corner = np.array(
