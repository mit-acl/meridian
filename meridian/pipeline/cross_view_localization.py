...
# ---------------------------------------------------------------------------
#
# `build_candidates_from_match_result` now lives in
# `meridian.cross_view.candidates` (imported above) so the algorithm layer does
# not depend on pipeline-layer data types. `context_from_data` bridges the
# pipeline's `CrossViewLocalizationData` to the algorithm's `LocalizationContext`.


def context_from_data(data: CrossViewLocalizationData) -> LocalizationContext:
    """Build a `LocalizationContext` from pipeline `CrossViewLocalizationData`."""
    needs_crs_reprojection = (
    ...
    return LocalizationContext(
        ...
        aerial_img_scale=data.aerial_img_scale,
        local_to_pixel_fn=(
            data.aerial_local_to_pixel if data.geotiff_transform is not None else None
        ),
        pixel_to_utm_fn=data.aerial_pixel_to_utm if needs_crs_reprojection else None,
        gt_pose_data=data.gt_pose_data,
    )
